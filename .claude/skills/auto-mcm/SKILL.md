...
每个 model_build + verify 阶段必须通过以下门控，才能执行 advance。

### 门控 1 — 文献质量

进入 model_build 前，`memory/thought_process.md` 中必须记录：
- ≥ 2 篇支持所选模型方法的学术文献（含 DOI 或 URL）
- 每篇一句话说明：为何选此方法而非其他
...
每次运行模型代码后，Agent 必须自问并在 `thought_process.md` 中记录：
...
| 目标函数值 | 量级是否符合物理/经济直觉？ |
| 梯度 / 收敛 | 优化是否收敛？迭代次数正常？ |
| 极端值 | 结果中有无 `inf`、`nan`、1e300 量级的值？ |

任一项不通过 → 不得 advance，回到 model_build 修复。

### 门控 3 — 验证脚本结构

`verify_problem{N}_{type}.py` 末尾必须打印结构化报告，格式严格如下：

```
========== VERIFICATION REPORT ==========
Stage  : model_{N}_verify
Result : PASS  ← 必须是 PASS 或 FAIL
Checks :
  ✓ 约束满足性: 所有决策变量在可行域内
  ✓ 物理可行性: 结果符合物理量纲
  ✓ 数值稳定性: 无 inf/nan，收敛
...
若 `Result: FAIL`，流水线禁止 advance，必须修复。

### 门控 4 — LaTeX 编译重试

`final_compile` 阶段编译失败时，执行自动修复循环（最多 3 次）：

```
尝试 1: xelatex -interaction=nonstopmode main.tex
  失败 → 解析 "! LaTeX Error:" 或 "Undefined control sequence" 定位行号 → 修复
尝试 2: 重新编译
  失败 → 解析新错误，再次修复
尝试 3: 重新编译
  仍失败 → 向用户汇报具体错误，请求人工干预
```

### 门控 5 — 多问题一致性（并行模式）

所有 model_N_verify 完成后、进入 sensitivity_analysis 前，执行一致性检查：

- 各子问题使用的物理常数（重力加速度、空气密度等）是否统一？
- 数据预处理结果（清洗后的数据集）是否被各子问题共享引用，而非各自独立读取原始数据？
- 若发现不一致 → 记录差异，按最严格版本统一，重跑受影响的 verify。
