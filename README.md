...
认证方式（按优先级自动检测）：
- 设置 `OPENAI_API_KEY` → 通过 OpenAI API 生成（按 token 计费）
- 安装 OpenAI Codex 并登录 → 通过 Codex CLI 的 `$imagegen` 生成（订阅额度内免费）
- 两者均未配置 → 自动跳过（退出码 2），在 LaTeX 中留占位符，**流水线不中断**
...
Authentication is detected automatically (in priority order):
- `OPENAI_API_KEY` set → generates via OpenAI API (token-based billing)
- OpenAI Codex installed and signed in → routes to `$imagegen` inside Codex CLI (free within subscription)
- Neither configured → gracefully skips (exit code 2), leaves a `\missingfigure{}` placeholder, **pipeline continues uninterrupted**
